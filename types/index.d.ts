...
declare class Hls {
  ...
  on(
    ...
    callback: (event: K_BUFFER_APPENDING, data: Hls.bufferAppendingData) => void
  ): void;
  on(
  ...
  on(
    ...
    callback: (event: K_MANIFEST_LOADING, data: Hls.manifestLoadingData) => void
  ): void;
  on(
  ...
  on(
    event: K_FRAG_PARSING_PRIVATE_DATA,
    callback: (
      event: K_FRAG_PARSING_PRIVATE_DATA,
      data: Hls.fragParsingPrivateData
    ) => void
  ): void;
  on(
